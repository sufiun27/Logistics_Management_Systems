...
<div class="card">
    ...
    <form class="form-horizontal" action="{{ route('ttInformation.updateTtInformation', $tt->id) }}" method="POST">
        ...
        <div class="card-body">
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-9">
                    <select name="tt_site" id="tt_site" class="form-control">
                        ...
                        <option value="{{$exrter->ExpoterName}}" {{ $tt->tt_site == $exrter->ExpoterName ? 'selected' : '' }}>{{$exrter->ExpoterName}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="form-group row">
                <label for="tt_date" class="col-sm-3 col-form-label text-end">TT Date</label>
                <div class="col-sm-9">
                    <input
                        type="date"
                        name="tt_date"
                        ...
                        id="tt_date"
                        placeholder="TT Date"
                        value="{{ old('tt_date', isset($tt->tt_date) ? \Illuminate\Support\Carbon::parse($tt->tt_date)->format('Y-m-d') : '') }}"
                    />
                </div>
            </div>
